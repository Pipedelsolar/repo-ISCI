...
def mapear_region(region, regiones_dict):
    ...
    for key, value in regiones_dict.items():
        similitud = son_similares(region, key)
        if similitud > mejor_similitud:
            mejor_similitud = similitud
            mejor_region = value

    reg_print.append(f"{region}---->{mejor_region}")
    return mejor_region if mejor_similitud > 0 else None

dias = list(range(7, (31-13)*7, 7)) #el 29 es porque usamos hasta semana 29, ampliar
meses = [1,2,3,4,5,6]


def pre_filtred(df_name, upc=False, drop_intersex=True):
    ...
    df['FECHA_ING_any'] = pd.to_datetime(df['FECHA_INGRESO'], format='%d%b%Y')
    ...
    for index, row in df.copy().query('cama=="UPC"').iterrows():
        if pd.isna(row['fecha_tras_1']):
            row['days_upc'] = (row['FECHA_EGR'] - row['FECHA_ING']).days
            
        else:
            row['dias_en_ing'] = (row['fecha_tras_1'] - row['FECHA_ING']).days
    ...
    if upc:
        df['event'] = (df['fecha_upc'].notna()) & (df['VRS']==1)
    else:
...
def filtros_IH(df,dias=dias, meses=meses, semana_lower=28,semana_upper=42, upc=False, con_reemplazo=True):
    ...
    if 'VIVO' in(df_filtered.columns):
        print("Datos perdidos por muertes: " , df_filtered.query('VIVO=="NO"').shape[0])
        df_filtered = df_filtered.query('VIVO=="SI"').copy()
    ...
    df1.loc[(df1['FECHA_INMUNIZACION'] > df1['FECHA_ING']) & (df1.VRS ==1 ), 'FECHA_INMUNIZACION'] = pd.NaT 
    
    if upc:
        df1.loc[ (~df1['event']) & (df1['fecha_upc'].notna()), 'fecha_upc'] = pd.NaT # df1.loc[ (~df1['VRS']), 'fecha_upc'] = pd.NaT
        df1.loc[ (df1['VRS']==0) & (df1['FECHA_ING'].notna()), 'FECHA_ING'] = pd.NaT
    else:
        df1.loc[ (~df1['event']) & (df1['FECHA_ING'].notna()), 'FECHA_ING'] = pd.NaT
      
    if con_reemplazo:
        if upc:
            print('reemplazos n/a net inmunizado: ', df1[((df1['FECHA_ING'] - df1['FECHA_INMUNIZACION']).dt.days <= 7) & 
                                                     (df1['VRS']==1) & (~df1.FECHA_INMUNIZACION.isna())].shape[0])
            df1.loc[((df1['FECHA_ING'] - df1['FECHA_INMUNIZACION']).dt.days <= 7) & (df1['VRS']==1) & (~df1.FECHA_INMUNIZACION.isna()), 'FECHA_INMUNIZACION'] = pd.NaT
        else:
            df1.loc[((df1['FECHA_ING'] - df1['FECHA_INMUNIZACION']).dt.days <= 7) & (df1['event']) & (~df1.FECHA_INMUNIZACION.isna()), 'FECHA_INMUNIZACION'] = pd.NaT
    else:
        df1['vrs_preinmune'] = np.where((df1.VRS == 1) & (df1.FECHA_ING < df1.FECHA_INMUNIZACION)) 
    ...
    if upc:
        vrs = df_filtrado[df_filtrado.event == 1].sort_values(by='fecha_upc').drop_duplicates(subset=['RUN'], keep='first')
        ...
        vrs = df_filtrado[df_filtrado.event == 1].sort_values(by='FECHA_ING').drop_duplicates(subset=['RUN'], keep='first')
        df_novrs = df_filtrado[~df_filtrado.RUN.isin(vrs.RUN.unique())].drop_duplicates(subset=['RUN'], keep='first')
        df_filtrado = pd.concat([df_novrs,vrs])
...
def cortes(df_f,dias=dias, meses=meses, cortes = {17:'2024-04-21',18:'2024-04-28',19:'2024-05-05',20:'2024-05-12',
    ...
    for i, fecha in  cortes.items():
        ...
        if upc:
            df_corte.loc[(df_corte['fecha_upc']> fecha_dt), 'fecha_upc'] = pd.NaT
            df_corte['event'] = df_corte['fecha_upc'].notna()
...
def preprocess_model_IH(df_f,upc=False):
    ...
    subset=['RUN','SEXO','sex*prem','FECHA_NAC','group',
            ...
            'atypic_mom_age', 'INS_N_M', 'SEMANAS'
            ]
    ...
    if upc:
        cumpleaños_covs = cumpleaños_covs + ['fecha_upc']
    ...
    covariates_fijas = ['RUN','SEXO','sex*prem','group','prematuro_extremo', #'FECHA_NAC'
                  ...
                  'NOMBRE_REGION','is_rural','porcent_rural','atypic_mom_age', 'INS_N_M', 'SEMANAS'
                  ]
    ...
    if upc:
        fecha_evento = 'fecha_upc'
    else:
        fecha_evento = 'FECHA_ING'

    base = df_model[df_model.fecha==fecha_evento][['RUN','SEXO','sex*prem','group',
                                                  ...
                                                  'is_rural','porcent_rural','atypic_mom_age', 'INS_N_M', 'SEMANAS']].drop_duplicates(subset=['RUN'])
    ...
    covs_fijas_news = base_fp[['RUN','SEXO','group','prematuro_extremo','muy_prematuro','prematuro_moderado',
                               'prematuro','EDAD_M','INS_C_M','COMUNA','COMUNA_N','REG_RES','URBA_RURAL',
                               'Macrozona2','NOMBRE_REGION','is_rural','porcent_rural','atypic_mom_age', 'INS_N_M', 'SEMANAS']]
