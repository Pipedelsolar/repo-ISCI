...
## Spline function per Andersson TM, Dickman PW, Eloranta S, Lambert PC. Estimating and modelling cure in population-based cancer studies within the framework of flexible parametric survival models.  # nolint: trailing_whitespace_linter.
...
NR.beta_cons <- function (build, beta.ini, detail.beta, max.it.beta = 200, tol.beta = 1e-04, cons) { # nolint
  type <- build$type
  X <- build$X
  X.GL <- build$X.GL
  event <- build$event
