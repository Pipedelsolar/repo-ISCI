...
def pre_filtred(df_name, lrti_name ='LRTI_Flag'):
    ...
    df = (df
          .assign(
              ...
              fecha_nac = lambda x: pd.to_datetime(x['FECHA_NACIMIENTO'], infer_datetime_format=True),
              fechaIng_any = lambda x: pd.to_datetime(x['FECHA_INGRESO'], infer_datetime_format=True),
              fechaEgr = lambda x: pd.to_datetime(x['FECHA_EGRESO'], infer_datetime_format=True),
              FECHA_INMUNIZACION = lambda x: pd.to_datetime(x['FECHA_INMUNIZACION'], infer_datetime_format=True),
...
def filtros_IH_new(df,
    ...
    for key, df_f in dfs_fecha.items():
        ...
        if meses_inm == None:
            max_meses_inm = (fecha_dt.month - df_f.fechaInm.min().month)
            print('max_meses_inm =', max_meses_inm)
            meses_inm = range(0, max_meses_inm + 1)
            
        for m in meses_inm:
            nombre_columna = f'inm_{m}_meses'
            nombre_columna_bin = f'inm_mayor_{m}_meses'
            df_f[nombre_columna] = df_f['fechaInm'] + pd.DateOffset(months=m)
            df_f.loc[(df_f[nombre_columna]> fecha_dt), nombre_columna] = pd.NaT
            df_f[nombre_columna_bin] = 1 - df_f[nombre_columna].isna()
        ...
        df_f['fechaFinInm'] = df_f['fechaInm'] + pd.to_timedelta(duracion_dias_nirse, unit='D')
        df_f.loc[(df_f['fechaFinInm']>= fecha_dt), 'fechaFinInm'] = pd.NaT   
...
def filtros_IH_simple(df,
    ...
    df_filtrado_vrs = fechas_df['fechaIng_vrs'].copy()

    fechas_events = {'fechaInm':'inmunizado','fechaIng_vrs':'event_vrs'}
    
    dfs_fecha = {'fechaIng_vrs': df_filtrado_vrs}
...
def call_data_cox(path,til_week,group_age=False,weeks_inm=False,fin_inmune=False,cohort='2025',T_inicial=None,duracion_dias_nirse=180,eliminar_inmunes_pre_season=True,meses = [1,2,3,4,5,6], fecha_dt = pd.to_datetime('2025-09-30'),meses_inm=None,meses_inm_bool=True,fecha_cohort_in=None,fecha_cohort_out=None,ef_2024_in_2025=False):
    ...
    ##################################################### esto es buena idea pero las columnas de fechas no se guardan como datetime y hay que vovler a procesarlas, webiado
    # if path=='COHORTE_NIRSE_ACTUALIZADA_04_12_2025_ENCR.csv':
    #     df_pf = pd.read_csv(path_data/'df_pf_04_12_2025.csv')
    # else:
    ...
    _, _, df_f_vrs, df_f_upc = filtros_IH_new(df_pf,
                                            ...
                                            #   meses=meses, 
                                              ...
                                              T_inicial=T_inicial, 
                                              fecha_dt = fecha_dt,
                                              duracion_dias_nirse=duracion_dias_nirse,
                                              meses_inm=meses_inm,
    ...
    if meses_inm==None:
        max_meses_inm = (fecha_dt.month - df_f_vrs.fechaInm.min().month)
        print('max_meses_inm =', max_meses_inm)
        meses_inm = range(0, max_meses_inm + 1)
    ...
    subset = ['RUN',
          "fecha_nac","fechaIng_any","fechaEgr","FECHA_INMUNIZACION","fechaInm",'fechaFinInm',"fechaIng_vrs","fechaIng_vrs_Dall", #FECHAS
          ...
          "group",'eleg_2025','eleg_2024','eleg_group',"sexo","prematuro_extremo","muy_prematuro","prematuro_moderado","prematuro","region","Macrozona2", #covs birth
          'EDAD_M','INS_C_M','REG_RES','URBA_RURAL','COMUNA','COMUNA_N', #covs birth
          'atypic_mom_age', 'INS_N_M', 'SEMANAS', 'semana_nac','mes_nac_name','year_nac', #covs birth #,'vrs_preabril'
          "cama","days_upc","dias_en_ing","days_estad_vrs","days_estad_vrs_Dall", #UPC
          "is_rural","categori_macro","categori_regions","exp_rural","percent_poor","percent_poor_multidim","is_poor",'vrs_pre_campaña_2024','vrs_pre_campaña_2025','porcent_rural', #otras
    ...
    # base_df['fechaFinInm'] = base_df['fechaInm'] + pd.to_timedelta(duracion_dias_nirse, unit='D')
    # base_df.loc[(base_df['fechaFinInm']> pd.to_datetime(cohort+'-09-30')), 'fechaFinInm'] = pd.NaT
    ...
    fechas_var = ["fecha_nac",'fechaInm',"fechaIng_vrs","fechaIng_any","fechaIng_LRTI","fecha_upc_vrs",'fechaFinInm'] #'fechaIng_vrs_Dall','fecha_upc_vrs_Dall' 19-12-2025
    ...
    base_df.loc[(base_df['fechaInm']<0), 'fechaInm'] = 0
    base_df.loc[(base_df['fechaFinInm']<0), 'fechaFinInm'] = 0
    ...
    covariates = ['RUN',"VRS_D1","VRS_D1y3","VRS_Dall",'inmunizado',
                        "group",'eleg_2025','eleg_2024','eleg_group',"sexo","prematuro_extremo","muy_prematuro","prematuro_moderado","prematuro","region","Macrozona2", 
                        ...
                        'atypic_mom_age', 'INS_N_M', 'SEMANAS', 'semana_nac','mes_nac_name','year_nac', #'vrs_preabril',
                        "cama","days_upc","dias_en_ing","days_estad_vrs","days_estad_vrs_Dall", 
                        "is_rural","categori_macro","categori_regions","exp_rural","percent_poor","percent_poor_multidim","is_poor",'vrs_pre_campaña_2024','vrs_pre_campaña_2025','porcent_rural',
                        'days_upc_vrs','days_upc_vrs_Dall','event_upc','event_upc_Dall','event_vrs','event_vrs_Dall','event_any','event_LRTI']
    ...
    df_model = df_model.merge(base_df[covariates], on = 'RUN' , how='left').assign(group = lambda x: (x['eleg_'+ cohort] == 'CATCH_UP').astype(int))
    
    if fin_inmune:
        df_model['inmunizado'] = np.where((df_model['fecha'] == 'fechaInm') & (df_model['duration'].notna()), 1,
                                        np.where((df_model['fecha'] == 'fechaFinInm') & (df_model['duration'].notna()), 0, np.nan)
                                        )
    
    subset2 = ['RUN',"group",'eleg_2025','eleg_2024','eleg_group',"sexo","prematuro_extremo","muy_prematuro","prematuro_moderado",
           ...
           'URBA_RURAL','COMUNA','COMUNA_N','atypic_mom_age', 'INS_N_M', 'SEMANAS', 'semana_nac','mes_nac_name','year_nac',"cama",
           "is_rural","categori_macro","categori_regions","exp_rural","percent_poor",#'vrs_preabril',
           "percent_poor_multidim","is_poor",'vrs_pre_campaña_2024','vrs_pre_campaña_2025','porcent_rural',
    ...
    if fin_inmune:
        ...
        df_0_na = add_covariate_time(df_0_na, df_model, 'fechaFinInm', 'inmunizado', 'event_vrs',stop_max)
        df_0_vrs = add_covariate_time(df_0_vrs, df_model, 'fechaFinInm', 'inmunizado', 'event_vrs',stop_max)
        df_0_upc = add_covariate_time(df_0_upc, df_model, 'fechaFinInm', 'inmunizado', 'event_upc',stop_max)
    ...
    if meses_inm_bool:
        for m in meses_inm: 
            fecha_d = f'inm_{m}_meses'
            binaria_d = f'inm_mayor_{m}_meses'
            df_0_na = add_covariate_time(df_0_na, df_model, fecha_d, binaria_d, 'event_vrs',stop_max)
            df_0_vrs = add_covariate_time(df_0_vrs, df_model, fecha_d, binaria_d, 'event_vrs',stop_max)
            df_0_upc = add_covariate_time(df_0_upc, df_model, fecha_d, binaria_d, 'event_upc',stop_max)
    ...
    for i, df_fe in enumerate(dfs_cox):
        fechas_var = ["fecha_nac","fechaInm","fechaIng_vrs",'fechaIng_any','fechaIng_LRTI',"fecha_upc_vrs",'fecha_evento','fechaFinInm'] #fechaIng_vrs_Dall fecha_upc_vrs_Dall 19-12-2025
        ...
        if fin_inmune:
            df_0 = add_covariate_time(df_0, df_model, 'fechaFinInm', 'inmunizado', 'evento', stop_max)
        ...
        if meses_inm_bool:
            for m in meses_inm: 
                fecha_d = f'inm_{m}_meses'
                binaria_d = f'inm_mayor_{m}_meses'
                df_0 = add_covariate_time(df_0, df_model, fecha_d, binaria_d, 'evento',stop_max)
...
def call_data_cox_matched(path,
                        #   til_week,
                        #   group_age=                  False,
                        #   weeks_inm=                  False,
                          fin_inmune=                 False,
                        ...
                        #   meses =                     [1,2,3,4,5,6], 
                          ...
                          match_vars_exact=           ["sexo", "region",'mes_nac_name','semana_nac','year_nac'],
                          base_covs =                 ["sexo","SEMANAS","region",'mes_nac_name','semana_nac','year_nac','inmunizado'] ):
    ...
    df_cases   = df_f_vrs[['RUN'] + base_covs].query("inmunizado == 0").copy() 
    df_control = df_f_vrs[['RUN'] + base_covs].query("inmunizado == 1").copy() 

    matched_base, _, _, _, _, _, _, _ = match_nn_max_dist_weigths( # unmatched, matched_controls, cases, controls, ratio, n_ctrl, n_case 
    ...
    df_vrs_matched = df_f_vrs.merge(match_map, on="RUN", how="inner").copy()
    # df_upc_tilweek = df_f_upc.copy()
    ...
    df_f_comun = df_vrs_matched.copy()
    ...
    '''
        # if i==0:
    # df_0 = pd.concat([df_0_list[0], df_0_list[1]])
            
        # elif i==1:
        #     df_0 = pd.concat([df_0.rename(columns={'evento': 'event_upc'}), df_0_list[0].copy().rename(columns={'event_vrs': 'event_upc'}), df_0_list[i+1]])
            
    # covs = pd.concat([covs_comunes[0], covs_comunes[1]])
    ...
    df_0_vrs_final = (pd.concat([df_0_list[0], df_0_list[1]])
                      .merge(pd.concat([covs_comunes[0], covs_comunes[1]]), on='RUN',how='left')
                      .copy())
            
        # elif i==1:
            # df_0_upc_final = df_0.copy()

    return df_0_vrs_final, df_vrs_matched, df_f_vrs # ,df_muertos, df_precampaña
...
def printSummary(ctv_0):
    ...
    cols.insert(0, cols.pop(cols.index('effectiveness')))
    cols.insert(1, cols.pop(cols.index('eff_lower_95')))
    cols.insert(2, cols.pop(cols.index('eff_upper_95')))
    cols.insert(3, cols.pop(cols.index('p')))
    sumary = sumary[cols]
...
def cox_return(df,covs,prematuros=False,splines=False):
    df_cox = df.copy()
    
    if splines:
        df_cox["cal_day"] = (df_cox["start"] + df_cox["stop"]) / 2

        spline = dmatrix("bs(cal_day, df=8, degree=3, include_intercept=False)",
                        data=df_cox, return_type="dataframe")

        df_cox = pd.concat([df_cox, spline.add_prefix("cal_spline_")], axis=1)

        spline_cols = spline.add_prefix("cal_spline_").columns.tolist()
        
        covs = covs + spline_cols
    ...
    if 'mes_nac_name' in covs:
        strata.append('mes_nac_name')
...
def call_data_cox_auxiliar(path,
                        til_week,
                        group_age=False,
                        weeks_inm=False,
                        fin_inmune=False,
    ...
    subset = ['RUN',
          ...
          "fecha_upc","fecha_upc_vrs","fecha_upc_vrs_Dall", 'fechaIng_LRTI','DIAG1', 'MARCA','PREVI','DIAG2','DIAG3','DIAG4','DIAG5','DIAG6','DIAG7','DIAG8','DIAG9','DIAG10', 'DIAG11',
          "VRS_D1","VRS_D1y3","VRS_Dall",'inmunizado', #VRS E INMINE
          "group",'eleg_2025','eleg_2024',"sexo","prematuro_extremo","muy_prematuro","prematuro_moderado","prematuro","region","Macrozona2", #covs birth
          'EDAD_M','INS_C_M','REG_RES','URBA_RURAL','COMUNA','COMUNA_N', #covs birth
          'atypic_mom_age', 'INS_N_M', 'SEMANAS', #covs birth
          "cama","days_upc","dias_en_ing","days_estad_vrs","days_estad_vrs_Dall", #UPC
          "is_rural","categori_macro","categori_regions","exp_rural","percent_poor","percent_poor_multidim","is_poor",'vrs_pre_campaña_'+cohort,'porcent_rural', #otras
          'days_upc_vrs','days_upc_vrs_Dall','event_upc','event_upc_Dall','event_vrs','event_vrs_Dall','event_any','event_LRTI']
    ...
    covariates = ['RUN',"VRS_D1","VRS_D1y3","VRS_Dall",'inmunizado','DIAG1', 'DIAG2','DIAG3','DIAG4','DIAG5','DIAG6','DIAG7','DIAG8','DIAG9','DIAG10', 'DIAG11',
                        "group",'eleg_2025','eleg_2024',"sexo","prematuro_extremo","muy_prematuro","prematuro_moderado","prematuro","region","Macrozona2", 
                        'EDAD_M','INS_C_M','REG_RES','URBA_RURAL','COMUNA','COMUNA_N', 
                        'atypic_mom_age', 'INS_N_M', 'SEMANAS', 'MARCA','PREVI',
                        "cama","days_upc","dias_en_ing","days_estad_vrs","days_estad_vrs_Dall", 
                        "is_rural","categori_macro","categori_regions","exp_rural","percent_poor","percent_poor_multidim","is_poor",'vrs_pre_campaña_' + cohort,'porcent_rural',
                        'days_upc_vrs','days_upc_vrs_Dall','event_upc','event_upc_Dall','event_vrs','event_vrs_Dall','event_any','event_LRTI']
    ...
    subset2 = ['RUN',"group",'eleg_2025','eleg_2024',"sexo","prematuro_extremo","muy_prematuro","prematuro_moderado",'DIAG1','DIAG2','DIAG3','DIAG4','DIAG5','DIAG6','DIAG7','DIAG8','DIAG9','DIAG10', 'DIAG11',
           ...
           "percent_poor_multidim","is_poor",'vrs_pre_campaña_' + cohort,'porcent_rural',
    ...
    print('IN preprocess VRS Y UPC')
    for i, df_fe in enumerate(dfs_cox):
        fechas_var = ['fecha_nac', "fechaInm","fechaIng_vrs",'fechaIng_any','fechaIng_LRTI',"fecha_upc_vrs",'fecha_evento']
        subset2.append('duration')
        base_df = df_fe.copy()[subset]
...
def plot_extended_kaplan_meier(df_cox,add_risk=False,log_rank_test=False,event='event_vrs'):
    
    print('this functions yields a plot KM and return 2 bbdd: df_mayer_vrs_agg, df_mayer_vrs')
    ...
    df_vrs_post = df_cox.copy()

    df_mayer_vrs = (
        df_vrs_post
        .loc[:, ['start', 'inmunizado', 'stop', 'RUN', event, 'region','sexo','SEMANAS','eleg_2024','eleg_2025','mes_nac_name','semana_nac']]
        .assign(duration = lambda x: x.stop - x.start)
    )

    aggre_vrs = (
        df_mayer_vrs
        .groupby(['RUN', event,'inmunizado'], as_index=False)
        .agg({'duration': 'sum'})  
    )


    df_mayer_vrs = pd.merge(aggre_vrs, df_mayer_vrs[['RUN', 'region','sexo','SEMANAS','eleg_2024','eleg_2025','mes_nac_name','semana_nac']].drop_duplicates('RUN'), how='left', on='RUN')

    if event=='event_vrs':
        df_mayer_vrs_agg = df_mayer_vrs[['RUN',event, 'duration', 'inmunizado']].groupby('RUN', group_keys=False).apply(process_RUN_vrs).reset_index(drop=True)
    elif event=='event_upc':
        df_mayer_vrs_agg = df_mayer_vrs[['RUN',event, 'duration', 'inmunizado']].groupby('RUN', group_keys=False).apply(process_RUN_upc).reset_index(drop=True)
    else:
        raise('not_implemented yet')
    
    resultados_log_rank = []

    T = df_mayer_vrs_agg["duration"]  
    E = df_mayer_vrs_agg[event]
    ...
    return df_mayer_vrs_agg, df_mayer_vrs


def call_data_cox_aux_2(path,
                        til_week,
                        group_age=False,
                        weeks_inm=False,
                        fin_inmune=False,
    ...
    return df_0_vrs_final, df_f_cox # ,df_muertos, df_precampaña
